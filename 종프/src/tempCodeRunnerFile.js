const { originalPool, groupedPool } = require('./database');
...
const insertGroupedDataIntoNewTable = async (groupedData) => {
  ...
  const insertPromises = groupedData.map(data => {
    ...
    return new Promise((resolve, reject) => {
      groupedPool.query(insertQuery, [data.country_of_residence, data.country_to_visit, data.age_group, data.gender, data.travel_preference, data.count], (err, results) => {
        if (err) {
